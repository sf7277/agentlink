...
AgentLink 是运行在用户可信 Mac 上的本地 Coding Agent 控制平面。它把移动端渠道连接到本机已登录的 Coding Agent，并统一管理项目、会话、审批、状态和服务生命周期。

当前提供个人实验与学习用途的 macOS 开发者安装方式，优先支持微信 ClawBot 与 Codex CLI；Grok 适配器可创建和管理 AgentLink 会话，但不支持导入既有 Grok 会话。
