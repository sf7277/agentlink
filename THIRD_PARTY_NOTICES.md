...
AgentLink 的 production 依赖来源于`package-lock.json`。每个构建发行目录保留各包自带的 LICENSE
文件，并生成 CycloneDX SBOM。本文是来源索引，不替代各依赖的原始许可证文本。
...
| 组件 | 版本 | 许可证 | 来源 |
|---|---:|---|---|
| @anthropic-ai/claude-agent-sdk | 0.3.220 | Anthropic PBC 专有条款，见该包 `LICENSE.md` | `pkg:npm/@anthropic-ai/claude-agent-sdk@0.3.220` |
...
精确组件、hash 和依赖关系以对应构建发行目录中的`sbom.cdx.json`为准；该文件是发行产物，不随源码
仓提交。
